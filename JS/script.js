...
const displayCardContainer = async (categoryId) => {
  try {
      ...
      div.innerHTML = `
        <div class="max-w-sm my-8 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          <a href="#" class="relative">
            <img class="rounded-t-lg w-full h-36" src="${cardInfo?.thumbnail}" alt="" />
            <span class="absolute bottom-3 right-3 bg-blue-400 rounded-sm text-white text-xs font-medium text-end p-1">
              ${cardInfo.others.posted_date > 0 ? `${hrs} hrs ${remainingMinutes} min ago` : ""}
            </span>
          </a>

          <div class="flex space-x-4 p-4">
            <img alt="" src="${cardInfo?.authors[0]?.profile_picture}" class="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500">
            <div class="flex flex-col space-y-1">
              <a rel="noopener noreferrer" href="#" class="text-sm font-semibold">${cardInfo?.title}</a>
              <p class="text-xs flex gap-2 items-center  dark:text-gray-400">
                ${cardInfo?.authors[0]?.profile_name}
                ${cardInfo?.authors[0].verified ? '<span><img class="w-4" src="./images/quality.png" alt="Verified" /></span>' : ""}
              </p>
              <p class="text-xs flex gap-1 items-center  dark:text-gray-400"><span>${cardInfo?.others?.views}</span> views</p>
            </div>
          </div>
        </div>
      `;
    ...
    console.error("Error displaying card container:", error);
  }
};

const sortData = async (sortCategory) => {
  try {
    const res = await fetch(`https://openapi.programming-hero.com/api/videos/category/${sortCategory}`);
    ...
    sortCardData.data.sort((a, b) => {
      ...
      return highestViews - lowestViews;
    });
  } catch (error) {
